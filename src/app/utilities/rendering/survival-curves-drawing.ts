 ...
 */
import { Selection, scaleLinear, ScaleLinear, axisBottom, event, axisLeft, scaleOrdinal, ScaleOrdinal, scaleBand, select, area, curveStepAfter, Line, Area, line, curveStepBefore, BaseType } from "d3"
...
export type confidenceInterval = (simga: number, point: { timePoint: number, prob: number, cumul: number, remaining: number }) => { inf: number, sup: number };

export const CIs: Array<{ label: string, value: confidenceInterval }> = [{ label: "identity", value: identity }, { label: "log", value: logarithm }, { label: "log -log", value: logarithmMinusLogarithm }, { label: "arcsine squared", value: arcsineSquaredRoot }]
...
export class SurvivalCurvesDrawing {
  ...
  // input points per group
  _survivalPoints: SurvivalPoint[][]
  ...
  constructor(svgRef: Selection<SVGGElement, unknown, HTMLElement, any>, private curves: SurvivalCurve, private width: number, private margins: number, private survivalTable: number, private height: number, public selectedInterval: confidenceInterval = logarithm, public alpha: number = 1.960, public nofTicks: number = 10, public grid: boolean = false) {
  ...
  private findMaxDomain(): number {
    var timePoints = this.curves.curves.map(curve => Math.max(...curve.points.map(point => point.timePoint)))
  ...
  buildLines() {
    ...
    this.curves.curves.forEach(((curve, index) => {
      this._groupButton.set(curve.groupId, this.toggleStyle(curve.groupId,
        this._svgRef.append("circle")
          ...
          .on("mousedown", (() => {
            var hidden = !this._hiding.get(curve.groupId)
    ...
    //curve references
    var curvePerSe = this._svgRef.append("g").attr("id", "resultDrawing")
    this.curves.curves.forEach(c => {
  ...
  buildTable() {
    ...
    var xaxis = scaleLinear().domain([-domainPadding, this.findMaxDomain() + domainPadding]).range([2 * this.margins, this.width - 2 * this.margins])
    var abscissa = axisBottom(xaxis).tickValues(this._ticks)
    var yaxis = scaleBand().domain(this.curves.curves.map(curve => curve.groupId)).range([this.height, this.survivalTable + 100])
    this._svgRef.append("g")
  ...
  drawCurves() {
    ...
    this.curves.curves.filter(curve => !this._hiding.get(curve.groupId)).forEach(curve => {
      ...
      var singlePointsEnterSelection = this._singlePoints.get(curve.groupId)
        .data(curve.points)
        .enter()
  ...
  changeGrid() {
    ...
    if (this._tableHGrid) {
      ...
      xaxis = scaleLinear().domain([-lefttDomainPadding, this.findMaxDomain() + rightDomainPadding]).range([2 * this.margins, this.width - 2 * this.margins])
      yaxis = scaleBand().domain(this.curves.curves.map(curve => curve.groupId)).range([this.height, this.survivalTable + 100])
      var abscissa = axisBottom(xaxis).tickValues(this._ticks)
